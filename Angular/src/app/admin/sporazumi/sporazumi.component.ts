...
@Component({
  ...
  styleUrl: './sporazumi.component.css'
})
export class SporazumiComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.loadAgreements(); // Load agreements if user is an admin
  }
  ...
  uploadAgreement(): void {
    ...
    this.http.post('http://localhost:5000/admin/upload_agreement', formData, { withCredentials: true }).subscribe(
      response => {
  ...
  updateAgreement(): void {
    ...
    this.http.put(`http://localhost:5000/admin/update_agreement/${this.selectedAgreement.id}`, formData, { withCredentials: true }).subscribe(
      response => {
  ...
  confirmDelete(agreementId: number): void {
    this.http.delete(`http://localhost:5000/admin/delete_agreement/${agreementId}`).subscribe(() => {
  ...
  loadAgreements(): void {
    this.http.get('http://localhost:5000/admin/get_agreements', { withCredentials: true }).subscribe(
      (response: any) => {
