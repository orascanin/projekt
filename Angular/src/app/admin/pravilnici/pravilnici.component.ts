...
@Component({
  selector: 'app-pravilnici',
  templateUrl: './pravilnici.component.html',
  styleUrl: './pravilnici.component.css'
})
export class PravilniciComponent implements OnInit{
  ...
  constructor(private authService: AuthService,private router: Router,private http: HttpClient,private modalService: NgbModal) {}

  ngOnInit(): void {
    this.checkLoginStatus();
    this.loadRegulations(); // Load regulations if user is an admin
  }
  ...
  uploadRegulation(): void {
    ...
    this.http.post('http://localhost:5000/admin/upload_regulation', formData, { withCredentials: true }).subscribe(
      response => {
  ...
  updateRegulation(): void {
    ...
    this.http.put(`http://localhost:5000/admin/update_regulation/${this.selectedRegulation.id}`, formData, { withCredentials: true }).subscribe(
      response => {
  ...
  confirmDelete(regulationId: number): void {
    this.http.delete(`http://localhost:5000/admin/delete_regulation/${regulationId}`).subscribe(() => {
      this.successMessage = 'Regulation deleted successfully';
      this.errorMessage = null;
      this.loadRegulations();  // Ponovo učitajte regulations nakon brisanja
      ...
      this.errorMessage = 'Error deleting regulation';
      this.successMessage = null;
      this.closeDeleteModal();
  ...
  loadRegulations(): void {
    this.http.get('http://localhost:5000/admin/get_regulations', { withCredentials: true }).subscribe(
      (response: any) => {
