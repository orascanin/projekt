...
@Component({
  selector: 'app-prikaz-pravilnici',
  templateUrl: './prikaz-pravilnici.component.html',
  styleUrl: './prikaz-pravilnici.component.css'
})
export class PrikazPravilniciComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this.loadProcurements(); // Dodajte ovo
  }
  ...
  loadProcurements(): void {
    this.http.get<any[]>('http://localhost:5000/guest/guest_get_regulations').subscribe(
      response => {
        this.regulations = response;
      },
      error => {
